#==============================================================================#
# PDP1: Recovery analyses for diff_delta
#==============================================================================#
...
# saves recovered parameter values at the hyper-parameter level for diff Delta
...
delta_file <- file.path(analysis_dir, 'simulated_choices_delta.xlsx')
delta_data <- read_xlsx(delta_file)
delta_DF <- as.data.frame(delta_data)

# harmonics at which data from same participant are separated
sample_size <- 30 # given how the output is organized two choices from the same subject is separated by the number of subjects
harmonic <- vector()
for (k in 0:100) {
  K <- k*sample_size
...
for (simulation in 1:10) {
  ...
  for (participant in 1:30) {
    ...
    # loops over trials
    for (trial in 1:100) {
      choice <- delta_DF[simulation,c(participant + harmonic[trial])]
      delta_sequence <- c(delta_sequence, choice)
      delta_sequence <- subset(delta_sequence, delta_sequence %in% c("1","2"))
      
    } # ends at trial 100
  } # ends at participant 30
  
  empirical_data <- data.frame(empirical_data, delta_sequence) # appends simulated sequence
...
# iteratively fits simulated data
...
write_xlsx(group_recovery,
           "C:\\Users\\Hubsc\\OneDrive\\Desktop\\PDPain_Study1\\Final_Analysis\\Results\\4_model_results\\Delta_group_recovery.xlsx",   # adapt
           col_names = TRUE)
